...
test("Version 4.6.1, Datenformat 9 und Datenbank 6 sind durchgängig verdrahtet", async () => {
  ...
  assert.equal(versionData.appVersion, "4.6.1");
  ...
  assert.match(config, /appVersion:\s*"4\.6\.1"/);
  ...
  assert.match(recovery, /const APP_VERSION = "4\.6\.1"/);
  assert.match(recovery, /const DATA_FORMAT_VERSION = 9/);
  assert.match(serviceWorker, /const APP_VERSION = "4\.6\.1"/);
});
