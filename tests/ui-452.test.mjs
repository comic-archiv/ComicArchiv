...
test("App-Version und Cache-Version sind synchron", async () => {
  ...
  assert.match(worker, /APP_VERSION\s*=\s*"4\.6\.10"/);
  assert.match(worker, /v4-6-10/);
  assert.match(recovery, /const APP_VERSION = "4\.6\.10"/);
  assert.equal(JSON.parse(metadata).appVersion, "4.6.10");
  assert.equal(JSON.parse(packageSource).version, "4.6.10");
});
